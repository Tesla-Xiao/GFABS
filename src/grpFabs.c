...
// Loss function
double Loss(y, X, b, status, model, sigma, param)
double *y, *X, *b, *sigma;
...
double bic(x, y, beta, status, set, sigma, sum_status, param)
double *x, *y, *beta, *sigma;
int *status, *set, *sum_status, *param;
{
	double score;
	int i, j, n, ns, count;
	...
	ns = param[3];
	count = ns;
	score = spr(y, x, beta, status, sigma, param)/n/(n-1);
	for (i = 0; i < ns; ++i) {
		if (set[i] % 2 == 1) {
			for (j = 0; j < ns; ++j)
		...
		SEXP Stoping, SEXP Iter, SEXP Param)
{
	...
	lossb        = Loss(y, x, b0, status, model, sigma, param);
	lossa        = Loss(y, x, beta, status, model, sigma, param);
	...
	for (i = 0; i < iter-1; ++i)
	{
		...
		lossb   = Loss(y, x, beta+i*p, status, model, sigma, param);
		lossa   = Loss(y, x, b0, status, model, sigma, param);
		...
		if ((descent < -1.0*xi) & (back == 1))
		{
			...
			lossb   = Loss(y, x, beta+i*p, status, model, sigma, param);
			lossa   = Loss(y, x, beta+i*p+p, status, model, sigma, param);
			normb   = mynorm(beta+i*p, k0[k], k1[k], type);
		...
		SEXP Stoping, SEXP Iter, SEXP Param)
{
	...
	PROTECT(Aset[0] = allocVector(INTSXP, param[3]));
	INTEGER(Aset[0])[0] = k+1;
	BI[0] = bic(x, y, beta, status, INTEGER(Aset[0]), sigma, sum_status, param);
	...
	for (i = 0; i < iter-1; ++i)
	{
		...
		BI[i+1] = bic(x, y, beta+(i+1)*p, status, INTEGER(Aset[i+1]), sigma, sum_status, param);
		if ( (stoping == 1) & (lambda[i+1] <= lambda[0] * lam_m) ) {
			i += 1;
			break;
		}
