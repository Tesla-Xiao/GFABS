...
fits = function(case, vc, cc, n,  p,  rho, error, tran) {
  ...
  result[1,]  = round(colMeans(VC), 2)
  result[2,]  = round(apply(VC, 2, sd), 2)
  result[3,]  = round(colMeans(CC), 2)
  result[4,]  = round(apply(CC, 2, sd), 2)
  result[5,]  = round(colMeans(ZC), 2)
  result[6,]  = round(apply(ZC, 2, sd), 2)
  result[7,]  = round(colMeans(ER), 2)
  result[8,]  = round(apply(ER, 2, sd), 2)
  result[9,]  = round(colMeans(IMSE), 2)
  result[10,] = round(apply(IMSE, 2, sd), 2)
  result[11,] = round(colMeans(Mvc), 2)
  result[12,] = round(apply(Mvc, 2, sd), 2)
  result[13,] = round(colMeans(Mnz), 2)
  result[14,] = round(apply(Mnz, 2, sd), 2)
